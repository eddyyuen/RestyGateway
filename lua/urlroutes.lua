...
function _M.match(uri)
	local match_url =nil
	local usejwt = true
	local server_uris_key= ""
	--local server_uri = nil
	local route = ""
	local  server_host = nil
	
	-- 查找URL是否符合正则
	for key, value in pairs(url_routes) do 
		if not value['route'] or value['route']=='' then
			route = "^"..value["prefix"].."([/0-9a-zA-Z-_]*)"
		else
			route = value['route']
		end
	...
	--取随机数
	--根据随机数获取serveruri
	local server_uris =cache.get(server_uris_key.."hosts")
